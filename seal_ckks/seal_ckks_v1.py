...
def c_times_c(c1, c2, rescale=True, relinearize=True):
    start = time.time()
    results = []
    for i in range(len(c1)):
        tmp_result = Ciphertext()
        ...
        results.append(tmp_result)
    print('C*C(Time,Rescale=%s,Relinearize=%s)' % (rescale, relinearize),
          (time.time() - start) / len(results) * 1000, 'ms')
    return results
...
for is_rescale in [True, False]:
    for is_relinearize in [True, False]:
        c_t_c = c_times_c(cipher_text, shuffled_cipher_text, rescale=is_rescale, relinearize=is_relinearize)
        c_t_c_p = decrypt(c_t_c, name='DecryptC*C(Time,Rescale=%s,Relinearize=%s)' % (is_rescale, is_relinearize))
        print('C*C(MAPE,Rescale=%s,Relinearize=%s)' % (is_rescale, is_relinearize),
